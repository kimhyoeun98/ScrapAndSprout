...
public class MiningMinigame : MonoBehaviour
{
    ...
    {
        // 내 캐릭터(HasInputAuthority)의 PlayerMovement 찾아서 잠금
        var players = FindObjectsByType<PlayerMovement>(FindObjectsSortMode.None);
        foreach (var pm in players)
        {
            if (!pm.HasInputAuthority) continue;
            if (lockMovement) pm.LockMovement();
            else pm.UnlockMovement();
            break;
        }
    }
    ...
    {
        _isPlaying = false; // 더 이상 입력 받지 않음
    ...
    void ShowFeedback(string text, Color color)
    {
        ...
        // 잠깐 보여주고 지우기
        StopAllCoroutines();
        StartCoroutine(ClearFeedbackAfter(0.4f));
    }
