...
public class AIBotController : NetworkBehaviour
{
    public enum BotState
    {
        ...
        MoveToTrash,
        CollectTrash,
        MoveToTeleporter,  // 텔레포터로 이동
        WaitTeleport,      // 텔레포트 대기
        SellTrash
    }
    ...
    void State_Idle()
    {
        if (_inSafeZone)
        {
            // SafeZone에 있으면 → 판매 후 TrashZone으로 이동
            if (_trashCollector != null && _trashCollector.inventory.Count > 0)
                ChangeState(BotState.SellTrash);
            else
                ChangeState(BotState.MoveToTeleporter); // TrashZone으로 돌아가기
        }
        else
        {
            // TrashZone에 있으면 → 쓰레기 채굴
            if (_trashCollector != null && _trashCollector.inventory.Count >= 3)
                ChangeState(BotState.MoveToTeleporter); // SafeZone으로 이동
            else
